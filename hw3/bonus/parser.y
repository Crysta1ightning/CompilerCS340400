...
%token<node> EXT_ADD EXT_SUB EXT_EQ EXT_LT
...
exprprec1: exprprec0 { $$ = $1; }
		...
		| EXT_ADD LEFT_PAREN argument COMMA argument RIGHT_PAREN {
			$$ = new BinaryExpression(op_extadd, $3, $5); 
			cleanup($1, $2, $4, $6);
		}
		| EXT_SUB LEFT_PAREN argument COMMA argument RIGHT_PAREN {
			$$ = new BinaryExpression(op_extsub, $3, $5); 
			cleanup($1, $2, $4, $6);
		}
		| EXT_EQ LEFT_PAREN argument COMMA argument RIGHT_PAREN {
			$$ = new BinaryExpression(op_exteq, $3, $5); 
			cleanup($1, $2, $4, $6);
		}
		| EXT_LT LEFT_PAREN argument COMMA argument RIGHT_PAREN {
			$$ = new BinaryExpression(op_extlt, $3, $5); 
			cleanup($1, $2, $4, $6);
		}
		;
